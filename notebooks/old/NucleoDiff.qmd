...
```{r}
outDirHu = "../results/NucleotideDifferential/Hu/"
dir.create(outDirHu, recursive=TRUE, showWarnings=FALSE)
outDirZhou = "../results/NucleotideDifferential/Zhou/"
dir.create(outDirZhou, recursive=TRUE, showWarnings=FALSE)
```
# Extract the NUCLEOTIDE METABOLISM genes
```{r}
#' Convert gene entrez ID to gene symbol using mygene, then extract the NUCLEOTIDE METABOLISM.
#' 
...
#' @examples 
#' # getNucleotide(taskInfo2)
#' 
getNucleotide = function(taskInfo2) {
    ...
    head(taskInfo2)
    nucleotide = taskInfo2 %>% filter(Depth1=="NUCLEOTIDE METABOLISM")
    return(nucleotide)
}
...
nucleotideHu = getNucleotide(taskInfo2)
nucleotideHu = nucleotideHu %>% unique()
saveRDS(nucleotideHu, paste0(outDirHu, "/nucleotideTaskInfoHu.RDS"))
openxlsx::write.xlsx(list(Hu2025=nucleotideHu),
    paste0(outDirHu, "nucleotideTaskInfoHu.xlsx"))
...
head(taskInfo2)
nucleotideZhou = getNucleotide(taskInfo2)
nucleotideZhou = nucleotideZhou %>% unique()
saveRDS(nucleotideZhou, paste0(outDirZhou, "/nucleotideTaskInfoZhou.RDS"))
openxlsx::write.xlsx(list(Zhou2020=nucleotideZhou),
    paste0(outDirZhou, "nucleotideTaskInfoZhou.xlsx"))
head(nucleotideZhou)
rm(taskInfo2, OUTDIR)
gc()
...
OUTDIR = "../results/CellFie_Hu2025/"
NdatHu = readRDS(paste0(OUTDIR, "/Pdat2.RDS"))
nucleotideHu = readRDS(paste0(outDirHu, "/nucleotideTaskInfoHu.RDS"))
...
sharedGenes = 
    intersect(as.character(unique(nucleotideHu$GeneAssociatedToEssentialRxnsTask)), rownames(NdatHu))
...
mappingHu = setNames(nucleotideHu$GeneAssociatedToEssentialRxnsTask_symbol,
    nucleotideHu$GeneAssociatedToEssentialRxnsTask)
...
OUTDIR = "../results/CellFie_Zhou2020/"
NdatZhou = readRDS(paste0(OUTDIR, "/Pdat2.RDS"))
nucleotideZhou = readRDS(paste0(outDirZhou, "/nucleotideTaskInfoZhou.RDS"))
...
sharedGenes = 
    intersect(as.character(unique(nucleotideZhou$GeneAssociatedToEssentialRxnsTask)), rownames(NdatZhou))
...
mappingZhou = 
    setNames(nucleotideZhou$GeneAssociatedToEssentialRxnsTask_symbol,
    nucleotideZhou$GeneAssociatedToEssentialRxnsTask)
...
getCor = function(
    ...
    ann_row = nucleotideInfo %>% dplyr::select(Depth3, GeneAssociatedToEssentialRxnsTask_symbol) %>%
        na.omit() %>% unique() %>% 
...
getCor(limma_rslt=limma_rslt_listZhou, 
    outdir=outDirZhou, 
    nucleotideInfo=nucleotideZhou,
    h=9, w=7)

getCor(limma_rslt=limma_rslt_listHu, 
    outdir=outDirHu, 
    nucleotideInfo=nucleotideHu,
    h=8, w=5.5)
